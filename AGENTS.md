...
**ASPは A8.net を使う**（メディアID `a26072638566`。もしもアフィリエイトは未登録）。
**2026-09-04時点では未稼働。** `assets/ads.js` の `AD_TAGS` が空なので、広告枠は DOM ごと削除され、
外部への通信もラベル表示も発生しない。

**稼働までに残っている作業は2つ。**

1. **A8に `misefits.kokokikaku.com` をサイト登録する。** 登録情報 → サイト情報の登録・修正。
   このページは**ログインID＋パスワードの再認証を要求する**ので、人がやるしかない。
   現在の登録サイトは note と「気になるモノ手帖」3件だけで、**MiseFits は入っていない**。
   提携申請の画面は「広告掲載予定のメディアを選択してください」と審査対象サイトを選ばせるため、
   **登録前に申請すると別サイトで審査されてしまう**。登録外サイトへの掲載はA8の禁止事項で、
   提携解除・成果全否認の対象になる。順番を飛ばさないこと。
2. 登録・提携承認のあと、**掲載サイトに MiseFits を選んで**広告リンクを生成し、
   下の `AD_TAGS` に貼る。貼ったら Console に CSP 違反が出ていないことを確認する。
...
**公開後にやること。** A8の「広告掲載URL管理」から、広告を載せたページのURLを提出する
（広告主の規約で求められている）。運営者・連絡先の記載義務は既存の特商法表記でクリア済み。
...
- **案件はページの文脈で出し分ける。** 割り当ては `ads.js` の `SLOTS` に書いてあり、
  ...
  | ページ | 広告主 | 理由 |
  |---|---|---|
  | `madori-2d.html` | 家具350（提携済み・購入8%） | 住まい寄り。家具を実寸で置く話と直結する |
  | それ以外の集客ページ | オフィスコム（未提携・購入5%※〜2026/09/30、通常3%） | 店舗・オフィス什器。EPC 214・確定率91%と数字が良い |
