...
> クラウド保存（サブスク）も2026-08-31に提供取りやめを決定した（下記「課金設計」の決定ブロックを参照）。
...
> **【2026-08-31決定】サブスク「クラウド保存」は提供しない。有料機能は買い切りのみとする。**
> 理由は3点。(1) Web版の有料販売を開始したのが2026-08-31で、需要がまだ1件も観測できていない。
> (2) 端末をまたぐ同期には本人を識別する仕組みが必須で、「アカウント登録・ログイン不要」という
> 既存の商品説明（`pro.html` / `faq.html`）と正面から衝突する。`localStorage` の `deviceId` に
> 紐づく現行のライセンス方式は同期の土台に使えない。(3) 認証・定期課金・解約導線・支払い失敗時の
> 処理・解約後のデータ保持と削除まで必要になり、静的サイト＋関数3本という現在の運用規模に対して重い。
> 公開ページ（`faq.html` の解約ルール、`tokushoho.html` の価格と解約条項）からは記述を削除し、
> `releases.html` で取りやめを告知済み。以下の設計は判断の経緯として残す。
...
- **【見送り】サブスク「クラウド保存」（自動更新・月額¥480／年額¥4,800）**：複数端末間の同期・バックアップのみ。
